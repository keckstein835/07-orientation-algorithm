...
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
function [V_orientation] = Image_grid_3D_FFT_KNE(imageStack, mask, crop_center_Location_voxels, cropWidth_voxels, voxel_size)
...
do_preset_filepath = 0; % Default to accept an imageStack when function is called (but can use a preset image that is pathed in this code)
do_primary_plots = false; % Default to suppress primary plots
...
if nargin < 3 % If only two inputs,
    crop_center_Location_voxels = [-4, -30, -8]; % Default (good for Xbox and others)
end
if nargin < 4 % If only three inputs,
    cropWidth_voxels = 20; % Default crop width in mm 
end
if nargin < 5 % If only four inputs,
    voxel_size = 0.5; % Default voxel size in mm
end

disp(['Image_grid_3D_FFT_KNE.m started: {time:} ' char(datetime("now", "Format", "yyyy-MM-dd HH:mm:ss")) ' [xyz] = ' num2str(crop_center_Location_voxels)]);
...
%% User settings
    ...
    percentile_ProminentFreqs = 99; % percentile of the most prominent frequencies, within feature size filter, to use as candidates for orientation vector voting procedure
        % higher percentile will make code go faster but may miss the correct orientation vector if the FFT signal is weak
    threshold_percentile = 99; % percentile of frequencies to use in template fitting (not sure this is necessary, as the strongest freq's should dominate, but here it is anyway)
...
cropStartX = round((size(imageStack, 1) - cropWidth) / 2) + crop_center_Location_mm(1) / voxel_size;
cropStartY = round((size(imageStack, 2) - cropWidth) / 2) + crop_center_Location_mm(2) / voxel_size;
cropStartZ = round((size(imageStack, 3) - cropWidth) / 2) + crop_center_Location_mm(3) / voxel_size;
...
numProminentFreqs = 4;
[V_1, lambda, mostProminentIndex, mostProminentFreq]  =  findMostProminentFrequency(filteredPowerSpectrum, Fx, Fy, Fz, voxel_size, numProminentFreqs);
...
mostIntenseOrientation = findMostIntenseOrientation(powerSpectrum_templatefit_input, rotated_dot_grids, candidateOrientationVectors, Fx, Fy, Fz, roll_increment_deg,do_primary_plots);
