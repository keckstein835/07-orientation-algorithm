...
%
% Description:
% Can be run as function, for example from the main file "Find_3D_lattice_fiber_dir_KNE.m",
%  or can be run from here as a standalone script.
...
%
% Syntax:  [V_orientation, FA] = Image_grid_3D_FFT_KNE(imageStack, mask, crop_center_Location_voxels_from_origin, cropWidth_voxels, voxel_size)
%
% Inputs:
%    imageStack - 3D matrix representing the image stack to be analyzed.
%    mask - 3D binary matrix of the same size as imageStack, used to mask the region of interest (and save time).
%    crop_center_Location_voxels_from_origin - 3-element vector specifying the center of the crop region in voxel coordinates.
%    cropWidth_voxels - Scalar specifying the width of the crop region in voxels.
...
function [V_orientation, FA] = Image_grid_3D_FFT_KNE(imageStack, mask, crop_center_Location_voxels_from_origin, cropWidth_voxels, voxel_size)
...
%% Check for input arguments and set defaults; clear variables if running standalone (no inputs)

if nargin < 2 % If one or no inputs,
    close all % ... close all figures
    disp('Running standalone with no inputs. File path is currently hard-coded.');
    
    % OK to change these here!
    do_primary_plots = true; % Default to display primary plots
    do_secondary_plots = true;
    do_preset_filepath = 1; % the alternative is imageStacdk is imported as an input
end
if nargin < 3 % If only two or less inputs,

    % *** if running standalone, set the crop center location here ***
    % crop_center_Location_voxels = [-4, 6, -8]; 
    % crop_center_Location_voxels = [0, 0, 0]; % Default (good for Xbox and others)
    % crop_center_Location_voxels = [1 6 31]-[60 60 36]  %case that was causing problems
    % crop_center_Location_voxels = [26  26  56] - [60 60 36]; % case that was causing problems
    crop_center_Location_voxels = [-2, -8, -8]


    crop_center_Location_voxels_from_origin = crop_center_Location_voxels + [60 60 36];


end
if nargin < 4 % If only three or less inputs, which must include the crop location
    cropWidth_voxels = 20; % Default crop width in mm (20 is a good place to be. Might need to be an even number, just FYI) 
end
if nargin < 5 % If only four or less inputs,
    voxel_size = 0.5; % Default voxel size in mm
end

% If this function is called externally with proper inputs:
if nargin > 2
    do_secondary_plots = false;
    do_primary_plots = false;
    do_preset_filepath = 0;
    crop_center_Location_voxels = crop_center_Location_voxels_from_origin-[60 60 36];
end

disp(['Image_grid_3D_FFT_KNE.m started: {time:} ' char(datetime("now", "Format", "yyyy-MM-dd HH:mm:ss")) ' [xyz] = ' num2str(crop_center_Location_voxels_from_origin)]);
...
%% User settings
    ...
    dot_radius_factor = 0.9; % Set to 1 for a reasonable dot radius for template fitting, but can reduce if needed for accuracy. 0.9 seems good.
    
    high_pass_cutoff_mm = 3; % mm; cutoff for high-pass filter to remove low-frequency noise (3 mm seems good)
...
cropWidth_mm = cropWidth_voxels*voxel_size; % mm; kernel width(?)
cropWidth = cropWidth_mm / voxel_size; % (yes I know these two lines are redundant but I'm keeping them for clarity)

% crop_center_Location_mm = crop_center_Location_voxels*voxel_size; % mm; kernel width(?)

% Add padding to the imageStack and mask to allow crops centered on edges
paddingSize = ceil(cropWidth / 2);
...
paddedMask = padarray(mask, [paddingSize, paddingSize, paddingSize], 0, 'both');
...
% Calculate the starting indices for cropping, offset by crop_center_Location_mm
% cropStartX = round((size(paddedImageStack, 1) - cropWidth) / 2) + crop_center_Location_voxels(1);
% cropStartY = round((size(paddedImageStack, 2) - cropWidth) / 2) + crop_center_Location_voxels(2);
% cropStartZ = round((size(paddedImageStack, 3) - cropWidth) / 2) + crop_center_Location_voxels(3);

cropStartX = (crop_center_Location_voxels(1)) + round((size(paddedImageStack, 1)) / 2 - cropWidth/2);
cropStartY = (crop_center_Location_voxels(2)) + round((size(paddedImageStack, 2)) / 2 - cropWidth/2);
cropStartZ = (crop_center_Location_voxels(3)) + round((size(paddedImageStack, 3)) / 2 - cropWidth/2);


% Ensure the starting indices are within the valid range
cropStartX = max(1, min(cropStartX, size(paddedImageStack, 1) - cropWidth ));
cropStartY = max(1, min(cropStartY, size(paddedImageStack, 2) - cropWidth ));
cropStartZ = max(1, min(cropStartZ, size(paddedImageStack, 3) - cropWidth ));

% Crop the DICOM stack
imageStack = paddedImageStack(cropStartX:cropStartX+cropWidth-1, cropStartY:cropStartY+cropWidth-1, cropStartZ:cropStartZ+cropWidth-1);
% Crop the mask too
mask = paddedMask(cropStartX:cropStartX+cropWidth-1, cropStartY:cropStartY+cropWidth-1, cropStartZ:cropStartZ+cropWidth-1);
...
if ( sum(mask(:)) < length(mask(:))*0.10 || isnan(sum(imageStack(:))) )
...
if do_primary_plots
    ...
    figure;
    crop_midslice = crop_center_Location_voxels(3)+ceil(size(originalImage,3)/2);
    % crop_slice = floor(size(originalImage, 3) / 2)+cropStartZ-paddingSize+1;
    
    imagesc(squeeze(originalImage(:, :, crop_midslice)));
    ...
    rectX = cropStartY-paddingSize ;
    rectY = cropStartX-paddingSize ;
